...
struct IncrementalTokenizerTests {
    ...
    func testTokenTypePreservation() throws {
        ...
        for i in 0..<min(originalTypes.count, newTypes.count) {
            if originalTokens[i].lexeme != "42" {
                #expect(originalTypes[i] == newTypes[i], "Non-modified tokens should preserve type")
            }
        }
    }
    ...
    func testLargeInsertionEfficiency() throws {
        ...
        var largeInsertion = ""
        for i in 0..<50 {
            largeInsertion += "変数 inserted\(i): 整数型 ← \(i)\n"
        }
    ...
    func testPerformanceRegressionDetection() throws {
        ...
        var document = ""
        for i in 0..<50 {
            document += "変数 variable\(i): 整数型 ← \(i * 2)\n"
        }
        ...
        for i in 0..<numberOfEdits {
            // Insert a small change
            let insertionPoint = currentText.index(currentText.startIndex, offsetBy: min(i * 10, currentText.count))
            let insertionRange = insertionPoint..<insertionPoint
            let newText = " // edit \(i)"
