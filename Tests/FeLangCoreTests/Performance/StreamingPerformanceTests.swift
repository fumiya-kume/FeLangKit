...
struct StreamingPerformanceTests {
    ...
    func testMemoryEfficiencyUnderPressure() async throws {
        ...
        for i in 1..<memoryGrowth.count {
            let sizeRatio = Double(sizes[i]) / Double(sizes[i-1])
            let memoryRatio = Double(memoryGrowth[i]) / Double(memoryGrowth[i-1])
    ...
    func testIncrementalEfficiencyUnderRepeatedEdits() throws {
        ...
        for i in 0..<50 {
            let insertionPoint = currentText.index(currentText.startIndex, offsetBy: min(i * 20, currentText.count))
