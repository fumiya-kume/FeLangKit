...
struct BenchmarkTests {
    ...
    func testDevelopmentFileParsingPerformance() throws {
        ...
        let _ = targetParseTime * 0.01 // 1.5ms for 10KB (not enforced in dev mode)
    ...
    func testComponentPerformanceBreakdown() throws {
        ...
        let _ = try StatementParser().parseStatements(from: tokens)
    ...
    func testLinearPerformanceScaling() throws {
        ...
        for i in 1..<timings.count {
            let ratio = timings[i] / timings[i-1]
...
extension BenchmarkTests {
    ...
    private func generateDeeplyNestedFile(depth: Int) -> String {
        ...
        for i in 0..<depth {
            content += String(repeating: "    ", count: i)
            content += "if condition\(i) then\n"
        }
