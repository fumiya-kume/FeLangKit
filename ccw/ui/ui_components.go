...
// ClearScreenBubbleTea provides Bubble Tea-compatible screen clearing
func (ui *UIManager) ClearScreenBubbleTea() tea.Cmd {
...
func (ui *UIManager) displayProgressHeader() {
	...
	if ui.isConsoleMode() {
		...
		// Screen clearing handled by Bubble Tea when used
...
// NOTE: This method is deprecated in favor of Bubble Tea lifecycle management
func (ui *UIManager) renderHeaderWithFlickerReduction(content string) {
	// In Bubble Tea mode, content rendering is handled by the program lifecycle
	// For legacy compatibility, just output content without cursor manipulation
	fmt.Print(content)
}
